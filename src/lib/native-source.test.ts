...
describe('native navigation source', () => {
  ...
  it('keeps the iOS WebView background synced with the loaded page', () => {
    ...
    expect(source).toContain('const routeEvents = ["hashchange", "popstate", "pageshow", "visibilitychange"]');
    expect(source).toContain('window.addEventListener(eventName, schedule)');
    expect(source).toContain('let usedFallbackColor = false');
    expect(source).toContain('if (usedFallbackColor) return');
